...
В режиме follow в терминале: Ctrl+C обрывает поток — локально dq завершает дерево процессов docker compose, при удалённом compose закрывается SSH-сессия (SIGINT в сам docker compose logs не пересылается). Ctrl+D не выходит из follow.
