...
Default deploy_engine is compose (backward compatible). Set deploy_engine to auto or api for Docker Engine API apply via SSH docker.sock tunnel (artifacts only); auto falls back to compose on failure. deploy_skip_unchanged and deploy_layer_sync optimize image transfer when API tunnel is available.`,
	...
	"clean.short": "Remove unused Docker artifacts for this compose project on the remote host",
	"clean.long": `Clean the remote host (remote_ssh / remote_path required) from unused Docker data belonging only to this project's compose_project_name.

Default: remove stopped containers, unused networks, and configured project images (deploy_image, deploy_images, images from compose file) that no running container uses. Other projects on the same server are not affected.

-a / --all-images: remove all unused tags of the same image repository (e.g. every unused downloadbot:*), not only the configured ref.

-v / --volumes: remove unused volumes labeled with this compose project (destructive if a DB volume is unreferenced).`,
	"clean.err.needs_remote": "clean requires remote_ssh and remote_path — it only cleans the remote host for this project",
	"clean.err.no_project":   "compose_project_name is empty",
	"clean.step.project":     "==> clean compose project %s on remote host\n",
	"clean.step.rmi":         "==> docker rmi %s\n",
	"clean.step.volume_rm":   "==> docker volume rm %s\n",
	"clean.flag.all_images":  "remove all unused tags of project image repositories (not only deploy_image)",
	"clean.flag.volumes":     "remove unused volumes for this compose project (destructive)",
